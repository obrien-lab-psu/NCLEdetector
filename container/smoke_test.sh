#!/usr/bin/env bash
# Smoke test for the NCLEdetector container image.
#
# Verifies the environment and bundled native tools are functional. Intended to
# run *inside* the container, e.g.:
#   docker run --rm ghcr.io/obrien-lab-psu/ncledetector:latest bash /opt/NCLEdetector/container/smoke_test.sh
#   apptainer exec ncledetector.sif bash /opt/NCLEdetector/container/smoke_test.sh
set -euo pipefail

echo "== NCLEdetector container smoke test =="
...
PKG=$(python -c "import NCLEdetector, os; print(os.path.dirname(NCLEdetector.__file__))")
...
python - <<'PY'
import NCLEdetector, topoly, mdtraj, MDAnalysis, freesasa, pyemma, numba, statsmodels
print("  ok: NCLEdetector, topoly, mdtraj, MDAnalysis, freesasa, pyemma, numba, statsmodels")
PY
