...
| workflow2 | Minimal Workflow 2 (CG last-67) | run_OP_on_simulation_traj.py | 10 | ~17 min | 3.5–30 min | Per-trajectory across 1000 CG trajectories (last 67 of 6667 frames), ops=[Q,G,K]; config `workflow2_OP_last67_config.json`. Variance driven by entanglement complexity per traj. |
| workflow2 | Minimal Workflow 2 (AA SASA/XP last-67) | run_OP_on_simulation_traj.py | 10 | ~43 min | 22–168 min | Per-trajectory across 1000 all-atom trajectories (last 67 frames), ops=[SASA,XP]; config `workflow2_OP_AA_last67_config.json`. Larger cost and spread than CG; stats use the per-file final run. |
| workflow2 | Minimal Workflow 3 | run_nonnative_entanglement_clustering.py | 8 | ~88 min | 62–114 min | 2 datastore runs: full trajectory set (61.8 min) and last-67-frame set (114.1 min); `nproc=8`. Runtime scales with number of detected entanglements, not frame count. |
