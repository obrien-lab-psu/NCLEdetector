## Benchmark Wall Times - P0AD61_4YNG_C

Template benchmark table for the CG benchmark test protein P0AD61 (4YNG, chain C). Fields are aligned with Documentation/Benchmarks.md. Fill wall times from successful NORMAL TERMINATION runs and application logs as needed.

| workflow | minimal_workflow | script | n_cpus | wall_time_avg | wall_time_range | notes |
|---|---|---|---|---|---|---|
| workflow1 | Minimal Workflow 1 | run_nativeNCLE.py | 1 | 5.6 min | 5.6 min-5.6 min | Completed in rerun job 54205932 (task 4). |
| workflow2 | Minimal Workflow 2 (CG full traj) | run_OP_on_simulation_traj.py | 8 | 46 min | 34 min-68 min | Completed for 50 trajectories (Q and chunked G outputs present). |
| workflow2 | Minimal Workflow 2 (CG last-67) | run_OP_on_simulation_traj.py | 8 | 21 min | 4.6 min-59 min | First-pass de novo estimate (base job 54185781 only); 48 completed trajectories in first pass. |
